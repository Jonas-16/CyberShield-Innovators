...
def _require_finite_scalar(value: float, *, stage: str, label: str) -> float:
    scalar = float(value)
    if not math.isfinite(scalar):
        raise ScannerStageError(stage, f"{label} is non-finite: {scalar}")
    return scalar
...
def _ml_scan(
    ...
    repaired_non_finite = int((~np.isfinite(features)).sum())
    if repaired_non_finite > 0:
        features = np.nan_to_num(features, nan=0.0, posinf=0.0, neginf=0.0)
        if not np.all(np.isfinite(features)):
            raise ScannerStageError("normalize_features", "feature vector still contains NaN or Inf values after repair")

    try:
        x = torch.tensor(features).unsqueeze(0).to(device)
        with torch.no_grad():
            static_prob = _require_finite_scalar(
    ...
    risk = _require_finite_scalar(
        _combine_scores(static_prob, None, fusion_alpha),
        stage="combine_scores",
        label="fused_risk",
    )
    reasons: list[str] = []
    ...
    if repaired_non_finite > 0:
        reasons.append("feature repair was required before ML inference")
        if decision == "ALLOWED":
            decision = "UNCERTAIN"
    ...
    return {
        ...
        "reasons": reasons,
        "scanner_warning": (
            f"[normalize_features] repaired {repaired_non_finite} non-finite feature values before inference"
            if repaired_non_finite > 0
            else None
        ),
...
def scan_file(
    ...
    if not math.isfinite(payload["static_prob"]):
        payload["static_prob"] = None
    if not math.isfinite(payload["fused_risk"]):
        payload["fused_risk"] = None
    if log_event:
