...
def _normalize_image_result(payload: dict[str, Any], module: Any) -> dict[str, Any]:
    ...
    is_stego = raw_decision == "STEGO" or predicted_label.strip().lower() == "stego"
    is_clean = raw_decision == "COVER"
    normalized_decision = "ALLOWED" if is_clean else "UNCERTAIN"
    reasons = (
        ["no steganography detected with high clean confidence"]
        if is_clean
        else ["steganography detected or image confidence below clean threshold"]
    )
